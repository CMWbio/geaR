...
contigMD <- seqVCF_Header("/media/chris/PhD/Introgression/Plutella_filtered_noMAF.recode.vcf.gz")$contig
...
pops2 <- data_frame(Sample = samples[1:6], Population = rep(c("pop1", "pop2"), each = 3))
...
system.time(geaR::getDiversityStats(GDS, loci[1:1000], minSites = 0.0005, nCores = 6, pops = pops, ploidy = 2, pairwiseDeletion = TRUE))
# 170MB data
# user  system elapsed
# 0.086   0.062  25.106
...
## Codons
fastaName <- "~/Desktop/Pea Project/References/GCA_000330985.1_DBM_FJ_V1.1_genomic.Pxmt.fasta"
gffName <- "~/Desktop/Pea Project/References/P.xylostella_exon.gff"
gffName <- "~/Desktop/Homo_sapiens.GRCh38.93.chromosome.13.gff3"
